    ...
    for elements in list_object:
        ...
        for key, value in elements.items():
            if isinstance(value, datetime):
                item[key] = value.strftime(MFCR_DATE_FORMAT)
                continue
...
def get_parsers(supported_parsers, parsers, input_dir=None):
    ...
    for parser in parsers:
        ...
        supported_parsers.append(
            (parser_name, supported_parsers[parser].Parser(input_dir))
        )
...
def get_unsupported_activity_types(supported_parsers, parser_statements):
    ...
    for parser_name, statements in parser_statements.items():
        parser_unsupported_activity_types = supported_parsers[
            parser_name
        ].get_unsupported_activity_types(statements)
